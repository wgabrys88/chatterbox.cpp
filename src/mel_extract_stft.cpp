...
std::vector<float> mel_graph_run(
    ...
    ggml_backend_t backend)
{
    ...
    ggml_tensor * t_frames  = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_fft, T);
    ggml_tensor * t_cos     = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_fft, F);
    ggml_tensor * t_neg_sin = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_fft, F);
    ggml_tensor * t_mel_fb  = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, F, n_mels);
    ...
    ggml_backend_buffer_t weights_buf = ggml_backend_alloc_ctx_tensors(ctx, backend);
    ...
    ggml_tensor * pow_ = ggml_add(gctx, ggml_sqr(gctx, spec_re), ggml_sqr(gctx, spec_im));
    ggml_tensor * mag = (power_exp == 1.0f) ? ggml_sqrt(gctx, pow_) : pow_;
    ggml_tensor * out = ggml_mul_mat(gctx, t_mel_fb, mag);
    if (log_floor > 0.0f) out = ggml_log(gctx, ggml_clamp(gctx, out, log_floor, 1e30f));
    ggml_set_name(out, "out"); ggml_set_output(out);
    ggml_build_forward_expand(gf, out);
    ggml_gallocr_t alloc = ggml_gallocr_new(ggml_backend_get_default_buffer_type(backend));
    ggml_gallocr_reserve(alloc, gf);
    ggml_gallocr_alloc_graph(alloc, gf);
    ggml_backend_tensor_set(t_frames, frames_TC.data(), 0, frames_TC.size() * sizeof(float));
    ggml_backend_graph_compute(backend, gf);
    std::vector<float> out_TM((size_t) T * n_mels);
    ggml_backend_tensor_get(ggml_graph_get_tensor(gf, "out"), out_TM.data(), 0, out_TM.size() * sizeof(float));
...
std::vector<float> fbank_kaldi_80(const std::vector<float> & wav_16k,
                                       ...
                                       ggml_backend_t backend)
{
    const int n_fft = 512, frame_len = 400, hop = 160, n_mels = 80;
    const int F = n_fft / 2 + 1;
    ...
    return mel_graph_run(build_kaldi_frames(wav_16k, T, hop, frame_len, n_fft, povey, preemph),
                         mel_fb, T, n_fft, F, n_mels, 2.0f, std::numeric_limits<float>::epsilon(), backend);
}
